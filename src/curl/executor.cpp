...
namespace asyncpp::curl {
	...
	int executor::exec_awaiter::await_resume() noexcept { return m_result; }
	...
	void executor::schedule(std::function<void()> fn, std::chrono::milliseconds timeout) {
		...
		if (m_thread.get_id() == std::this_thread::get_id()) {
			...
			std::sort(m_scheduled.begin(), m_scheduled.end(), [](const auto& a, const auto& b) { return a.first < b.first; });
		} else {
